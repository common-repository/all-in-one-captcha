...
<form method="POST" action="<?php echo esc_url( admin_url( 'options.php' ) ); ?>" class="aioc-settings">
	<input type="hidden" name="aioc-tab-id" value="aioc-login-settings" />
	<input type="hidden" name="aioc-form-key" value="aioc_login_options" />
	<?php wp_nonce_field( 'aioc-captcha-nonce', 'aioc-nonce' ); ?>

	<?php
	$form_fields = aioc_login_setting_fields();
	$count       = 1;
